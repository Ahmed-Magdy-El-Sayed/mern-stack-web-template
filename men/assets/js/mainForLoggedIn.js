...
const applyTheNotify = notif=>{//appear the new notification to the user
    ...
    if(content.classList.contains("show")){
        if(content.children.length-1)//if there is old notification
            ...
            <li class="mb-3"> <a class="text-dark text-decoration-none" href="${notif.href}">${notif.msg}</a> </li>
            `)
...
const changeProfile= e=>{//function for control inputs fields in update profile form
    const saveBtn = document.querySelector(".edit-profile .modal-footer button")
    if(e.target.value){//if the input not empty
        if(e.target.type == "password"){
            e.target.parentElement.parentElement.classList.add("was-validated")
            ...
            const passwordFeadback = changePassInputs[1].nextElementSibling
            changePassInputs[2].pattern = changePassInputs[1].value; // force the two password to be matched
            if(!changePassInputs[1].value.match(/.*[a-z]/g)) passwordFeadback.innerText = "password should contan at least one lowercase"
            else if(!changePassInputs[1].value.match(/.*\d/g)) passwordFeadback.innerText = "password should contan at least one number"
            else if(!changePassInputs[1].value.match(/.*[A-Z]/g)) passwordFeadback.innerText = "password should contan at least one uppercase"
            else passwordFeadback.innerText = "password should be at least 8 or more characters"
            changePassInputs.forEach(input=>{
                input.setAttribute('required',''); // make the two password fields required
            })
        }else{//for the input fields that not password
            e.target.setAttribute('required','')
        }
        saveBtn.disabled = false
    }else{//if one of the inputs is empty, undo the changes made above
        if(e.target.type == "password"){
            e.target.parentElement.parentElement.classList.remove("was-validated")
        ...
        if(! [...(document.querySelectorAll(".edit-profile input"))].map(input=>input.value?true:false).includes(true))
