...
const toggleUserEmailNotif = (req, res, next)=>{
    ...
    userModel.toggleEmailNotif(user._id, !user.emailNotif).then(()=>{
        ...
        req.session.save(()=>{
            res.cookie("user", JSON.stringify({...JSON.parse(req.cookies.user), emailNotif: user.emailNotif}), {expires: req.session.userSessionExp, path: "/"})
            res.status(200).end()
        })
    }).catch(err=> next(err))
}
        ...
        req.session.destroy(err=>{
            ...
            userModel.deleteUser(user._id).then(result=>{
                if(!result) res.status(400).json({msg:"Account not found!"})
                    ...
                    res.cookie("user", "", {expires: new Date("Thu, 01 Jan 1970 00:00:01 GMT"), path: "/"})
                    res.status(201).end()
                    sendEmail(user.email, {
                        title:"Your Account Was Deleted",
