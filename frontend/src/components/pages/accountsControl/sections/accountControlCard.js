...
function AccountControlCard({account, updateAccounts, mode}) {
    ...
    const changeAuthz = form=>{
        ...
        data.append("emailNotif", account.emailNotif)
        fetch(process.env.REACT_APP_API_SERVER+"/account/authzs/change", {
            ...
            if(res.ok){
                updateAccounts(accounts=>{
                    const index = accounts.findIndex(acc=> String(acc._id) === String(account._id))
                    account.role = data.get("authz");
                    return [
    ...
    const deleteAccount = e=>{
        ...
        fetch(process.env.REACT_APP_API_SERVER+"/account/delete",{
            ...
            if(res.ok){
                ...
                updateAccounts(accounts=>{
                    const index = accounts.findIndex(acc=> String(acc._id) === String(account._id))
                    return  accounts.slice(0, index).concat(accounts.slice(index+1))
                })
            }
            else throw await res.json();
        }).catch( err=>{
    ...
    return (
        <div className={`card text-center ${mode === 'dark'&& 'bg-dark border border-light text-light'}`} style={{ width: '18rem'}} id={`card-${account._id}`}>
