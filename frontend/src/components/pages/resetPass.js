...
export default function ResetPass() {
    ...
    const reset = e=>{
        ...
        refs.current.spinner.classList.remove("d-none")
        fetch(process.env.REACT_APP_API_SERVER+"/account/password-reset",{
            ...
            refs.current.spinner.classList.add("d-none")
        ...
        <div className="row w-100 justify-content-center text-center">
            ...
            <form className="reset-pass col-11 col-sm-6 needs-validation was-validated" onSubmit={reset}>
                ...
                <div className="password-container">
                    <input className="form-control mt-3" type="password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" name="password" placeholder="Enter the new password" onKeyUp={e=>{refs.current.invalidFeedback.nextElementSibling.pattern = e.target.value;refs.current.invalidFeedback.innerHTML = passwordValidation(e.target.value)}} required />
                    <div className="invalid-feedback main-password w-auto" ref={ref=>{refs.current.invalidFeedback = ref}}></div>
                ...
                <button className="btn btn-primary my-3" type="submit" onClick={addBsVal}>
                    <span className="spinner-border spinner-border-sm d-none me-1" ref={ref=>{refs.current.spinner = ref}} aria-hidden="true"></span>
                    Reset
                </button>
            </form>
        </div>
