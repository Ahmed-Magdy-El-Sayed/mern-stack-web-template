            ...
            <div class="card-body">
                ...
                (new Date().getTime() < account.ban?.ending)?
                    `<div class="ban-info">
                        ...
                        <p>The reason: ${account.ban.reason}</p>
                ...
                :
                    ...
                    <form class="collapse mt-3" id="ban${account._id}" action="/account/ban" method="post" onsubmit="sendBan(event)">
                        ...
                        <input class="form-control mb-2" type="number" min="1" name="duration" placeholder="Enter the ban duration (dayes)" required="required" />
                        <p>no fractions</p>
                        <input class="form-control mb-2" type="hidden" name="email" value="${account.email}"/>
...
const sendBan = e=>{
    ...
    if(parseInt(data.get("duration"))>=1){
        fetch(form.action,{
            ...
            <div class="ban-info">
                <p>this account is Banned until ${new Date(new Date().getTime()+parseInt(data.get("duration"))*24*60*60*1000).toLocaleString("en")}</p>
                <p>The reason: ${data.get("reason")}</p>
