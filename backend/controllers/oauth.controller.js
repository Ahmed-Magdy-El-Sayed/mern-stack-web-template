...
const postUsername = (req, res, next)=>{
  ...
  setUsername(user._id, username).then(()=>{
    ...
    req.session.save(()=>{
      res.cookie("user", JSON.stringify({...JSON.parse(req.cookies.user), username}), {expires: req.session.userSessionExp/* , secure: true, sameSite: "none" */, domain: new URL(process.env.REACT_APP_URL).hostname, path: "/"})
      res.status(201).end()
    })
  }).catch(err=> next(err))
}
